...
export default function rendererFactory(): IRenderComponent {
  ...
  class FaultComponent extends PureComponent<IPublicTypeNodeSchema | any> {
    render() {
      logger.error(
        `%c组件渲染异常, 异常原因: ${this.props.error?.message || this.props.error || '未知'}`,
        'color: #ff0000;',
      );
  ...
  return class Renderer extends Component<IRendererProps> {
    ...
    patchDidCatch(SetComponent: any) {
      ...
      SetComponent.prototype.componentDidCatch = function didCatch(
        this: any,
        error: Error,
        errorInfo: any,
      ) {
      ...
      SetComponent.prototype.shouldComponentUpdate = function (
        nextProps: IRendererProps,
        nextState: any,
      ) {
