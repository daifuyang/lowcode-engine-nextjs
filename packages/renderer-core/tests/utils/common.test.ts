...
describe('test isSchema', () => {
  ...
  it('should check each item of an array', () => {
    ...
    const invalidArraySchema = [...validArraySchema, { componentName: 'ComponentWithoutProps' }];
...
describe('test forEach ', () => {
  it('should work', () => {
    ...
    let sum = 0;
    const mockFn3 = function (value, key) {
      sum = value + this.b;
    };
...
describe('test serializeParams ', () => {
  it('should work', () => {
    const mockParams = {
      ...
      c: 'cvalue',
      d: [1, 'a', {}],
      e: { e1: 'value1', e2: 'value2' },
    };
    const result = serializeParams(mockParams);
    const decodedParams = decodeURIComponent(result);
    expect(result).toBe(
      'a=1&b=2&c=cvalue&d=%5B1%2C%22a%22%2C%7B%7D%5D&e=%7B%22e1%22%3A%22value1%22%2C%22e2%22%3A%22value2%22%7D',
    );
...
describe('test parseThisRequiredExpression', () => {
  ...
  it('[error] JSExpression handle without this use scopeValue', () => {
    ...
    expect(fn).toBeCalledWith(
      ...
      new ReferenceError('state is not defined'),
      { type: 'JSExpression', value: 'state.text' },
      { state: { text: 'text' } },
    );
  });
...
describe('test parseData ', () => {
  ...
  it('should work when schema is string', () => {
    expect(parseData(' this is a normal string, will be trimmed only ')).toStrictEqual(
